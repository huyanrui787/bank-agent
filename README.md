...
- 知识库：文件列表（拖拽/批量/文件夹上传、重命名、下载、启用禁用、重新解析、批量操作、chunk 分块管理）、检索测试（相似度/向量权重滑杆、来源文件过滤）、解析日志（状态汇总/详情）、切片与解析配置（自动关键词/问题、父级分块、PDF 布局识别等），背后调 RAGFlow 检索引擎（独立部署），AI 工作台语义检索并标注文件来源
