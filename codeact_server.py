...
_base_preamble = build_sqlite_preamble(DB_PATH, data_notes=_DB_NOTES)
...
def _sqlalchemy_url(ds: "DataSourcePayload") -> str:
    ...
    if t == "mysql":
        ...
        return f"oracle+cx_oracle://{user}:{pw}@{host}:{p}/?service_name={service}"
    elif t == "db2":
        p = port or 50000
...
def build_datasource_preamble(ds: "DataSourcePayload") -> str:
    ...
    t = ds.type
    if t in ("mysql", "postgresql", "sqlserver", "oracle", "db2", "hive", "impala", "vector_pgvector"):
        return _build_sqlalchemy_preamble(ds)
    elif t == "elasticsearch":
        return _build_es_preamble(ds)
    elif t == "vector_milvus":
        return _build_milvus_preamble(ds)
    elif t == "dtsql":
