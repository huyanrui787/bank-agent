// 「企业客户」独立演示库的数据（写入 data/enterprise.db）。
// 全部为虚构数据，不含真实企业信息。与 scripts/generate_enterprise_db.py 同源。
...
type RawCompany = [
  name: string, industry: string, scale: string, capitalWan: number,
  established: string, legalRep: string, taxLevel: string,
  riskLevel: EnterpriseCompany["riskLevel"], manager: string,
]
...
export const enterpriseCompanies: EnterpriseCompany[] = RAW_COMPANIES.map((c, i) => ({
  id: `E${String(i + 1).padStart(3, "0")}`,
  name: c[0],
  industry: c[1],
  scale: c[2],
  registerCapitalWan: c[3],
  established: c[4],
  legalRep: c[5],
  taxLevel: c[6],
  riskLevel: c[7],
...
const LOAN_TYPES = ["流动资金贷款", "固定资产贷款", "票据融资", "担保贷款"]
const STATUS = ["正常", "正常", "正常", "关注", "逾期"]

export const enterpriseLoans: EnterpriseLoan[] = (() => {
  const loans: EnterpriseLoan[] = []
  let n = 1
  enterpriseCompanies.forEach((_c, idx) => {
    const cid = `E${String(idx + 1).padStart(3, "0")}`
    for (let j = 0; j < 1 + (idx % 3); j++) {
      const amtWan = (5 + ((idx * 37 + j * 53) % 400)) * 10
      const rate = Math.round((3.5 + ((idx + j) % 20) * 0.05) * 100) / 100
      loans.push({
        id: `L${String(n).padStart(3, "0")}`,
        companyId: cid,
        loanType: LOAN_TYPES[(idx + j) % LOAN_TYPES.length],
        amount: amtWan * 10000,
        rate,
        startDate: `202${3 + (idx % 3)}-${String(1 + (idx % 12)).padStart(2, "0")}-${String(1 + (j % 20)).padStart(2, "0")}`,
        dueDate: `2026-${String(1 + ((idx + j) % 12)).padStart(2, "0")}-${String(1 + ((idx * 2 + j) % 27)).padStart(2, "0")}`,
        status: STATUS[(idx + j * 2) % STATUS.length],
      })
      n++
    }
  })
